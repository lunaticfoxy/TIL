...
- 이때 보스가 명령을 내렸을때 회사 전체에 퍼지는 시간을 
...
object Solution {
    def numOfMinutes(n: Int, headID: Int, manager: Array[Int], informTime: Array[Int]): Int = {
        def recurFunc(curNode: Int, informTime: Array[Int], children: Array[Array[Int]]): Int = {
            if(children(curNode).size == 0)
                0
            else
                children(curNode).map(i => recurFunc(i, informTime, children)).max + informTime(curNode)
        }
        
        val managerMap = (0 to manager.size - 1).map(i => (i, manager(i)))
            .groupBy(x => x._2)
        
        recurFunc(headID, informTime, (0 to manager.size).map{i => 
            if(managerMap.contains(i))
                managerMap(i).map(x => x._1).toArray
            else
                Array[Int]()
        }.toArray)
    }
