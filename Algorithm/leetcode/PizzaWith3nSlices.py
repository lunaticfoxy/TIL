...
class Solution:
    def maxSizeSlices(self, slices: List[int]) -> int:
        ...
        def maxSum(slices, max_pick):
            dp = [[0 for _ in range(max_pick+1)] for _ in range(len(slices)+1)]
        ...
        return max(maxSum(slices[:-1], int(len(slices)/3)), maxSum(slices[1:], int(len(slices)/3)))
