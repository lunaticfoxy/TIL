...
- case 2. 패턴 문자가 있을 경우
  ...
  - case 2-2. 채크할 문자가 있을 경우
    - case 2-2-1. 현재 패턴 문자가 . 이거나 체크할 문자와 일치할 경우
      - case 2-2-1-1. 현재 패턴 문자에 * 가 붙어 있을 경우
        - 다음 세가지 방법으로 재귀를 돌려 하나라도 true 라면 true 리턴
        - 현재 문자는 지우고 패턴 문자는 그대로 둔 뒤 재귀 수행
        - 현재 문자와 현재 패턴문자 + * 까지 지우고 재귀 수행
        - 현재 문자는 그대로 두고 현재 패턴문자 + * 만 지우고 재귀 수행
      - case 2-2-1-2. 현재 패턴 문자에 * 가 붙어 있지 않을 경우
        - 현재 문자와 현재 패턴문자를 지우고 재귀 수행
    - case 2-2-2. 현재 패턴문자가 .이 아니고 체크할 문자와 일치하지 않을 경우
      - case 2-2-2-1. 현재 패턴 문자에 * 가 붙어 있을 경우
        - 현재 문자는 그대로 두고 현재 패턴문자 + * 만 지우고 재귀 수행
      - case 2-2-2-2. 현재 패턴 무자에 * 가 붙어 있지 않을 경우
        - false 리턴
